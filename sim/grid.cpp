...
Grid::Grid(int size_x, int size_y, int size_z)
  ...
  for (int i = 0; i < this->size; ++i) { this->blocks.emplace_back(i, size_x, size_y, size_z); }
}

void Grid::populate(Simulation & sim, std::ifstream & inputFile) {
  int const num_p = sim.get_num_p();
...
void Grid::motion(Simulation & sim) {
  ...
  for (auto & block : this->blocks) {
    ...
    for (auto & particle : block.particles) {
      ...
      particle.velX = particle.hvX + half * particle.accX * sim.delta_t;
      particle.velY = particle.hvY + half * particle.accY * sim.delta_t;
      particle.velZ = particle.hvZ + half * particle.accZ * sim.delta_t;
